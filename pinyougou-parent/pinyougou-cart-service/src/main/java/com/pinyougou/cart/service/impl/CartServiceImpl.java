...
public class CartServiceImpl implements CartService {
	...
	@Override
	public List<Cart> addGoodsToCartList(List<Cart> carList, Long itemId, Integer num) {
		...
		Cart cart = searchCartBySellerId(carList, sellerId);
		if (cart == null) {// 4.如果购物车列表中不存在该商家的购物车
			...
			// 4.2讲新建的购物车对象添加到购物车列表
			cart.setOrderItemList(orderItemList);
			carList.add(cart);
		} else {// 5.如果购物车列表中存在该商家的购物车
			// 判断该商品是否在购物车的明细列表中
			TbOrderItem orderItem = searchOrderItemId(cart.getOrderItemList(), itemId);
			if (orderItem == null) {
				// 5.1如果不存在，创建新的购物车明细对象，并添加到该购物车的明细列表中
				orderItem = createOrderItem(item, num);
				cart.getOrderItemList().add(orderItem);
			} else {
				// 5.2如果存在，在原有的数量上增加数量，并且更新金额
	 ...
	 * @return
	 */
	public TbOrderItem searchOrderItemId(List<TbOrderItem> orderItemList, Long itemId) {
